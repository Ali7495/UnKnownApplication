...
{
    public class PersonValidation : AbstractValidator<PersonInputDto>
    {
        public PersonValidation()
        {
            RuleFor(r => r.Name).NotEmpty();
            RuleFor(r => r.LastName).NotEmpty();
            RuleFor(r => r.NationalCode).NotEmpty().Custom((code, context) =>
             {
                 if (!ValidateNationalCode(code))
                 {
                     context.AddFailure("National code is not valid");
                 }
             });
